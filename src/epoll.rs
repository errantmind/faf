...
fn threaded_worker(port: u16, cb: fn(*const u8, usize, *const u8, usize, *mut u8, *const u8) -> usize, cpu_core: i32) {
   ...
   let mut reqbuf: [u8; REQ_RES_BUFF_SIZE * MAX_CONN] = unsafe { core::mem::zeroed() };
   let mut reqbuf_cur_addr: [isize; MAX_CONN] = unsafe { core::mem::zeroed() };

   // Init request buffer state
   {
      let reqbuf_start_address = &mut reqbuf[0] as *mut _ as isize;
      (0..reqbuf_cur_addr.len()).for_each(|i| {
         reqbuf_cur_addr[i] = reqbuf_start_address + i as isize * REQ_RES_BUFF_SIZE as isize;
      });
   }
   ...
   loop {
      ...
      for index in 0..num_incoming_events {
         ...
         let req_buf_start_address = (&mut reqbuf[0] as *const u8 as isize).add(cur_fd * REQ_RES_BUFF_SIZE as isize);
         ...
         if cur_fd == listener_fd {
            ...
            let cur_address = unsafe { reqbuf_cur_addr.get_unchecked_mut(cur_fd as usize) };
            let buffer_used = *cur_address - req_buf_start_address;
            let buffer_remaining = REQ_RES_BUFF_SIZE as isize - buffer_used;

            let read = sys_call!(SYS_READ as isize, cur_fd, *cur_address, buffer_remaining);
            ...
            if likely(read > 0) {
               ...
               if request_buffer_offset == 0 || response_buffer_filled_total == 0 {
                  *cur_address = req_buf_start_address;
